...
<div class="card card-default">
  <div class="card-header">
    商品评价
    <a class="pull-right" href="{{ route('order.index') }}">返回订单列表</a>
  </div>
  <div class="card-body">
      ...
      <tr>
        ...
        <td>
          ...
          @if($order->reviewed)
            {{ $item->review }}
          @else
            <textarea class="form-control {{ $errors->has('reviews.'.$index.'.review') ? 'border-danger' : '' }}" name="reviews[{{$index}}][review]"></textarea>
            <!-- 如果评价数据验证不通过 ， 显示验证错误信息 -->
            @if($errors->has('reviews.'.$index.'.review'))
              @foreach($errors->get('reviews.'.$index.'.review') as $msg)
                <span class="help-block text-danger">{{ $errors->get('reviews.'.$index.'.review')[0] }}</span>
              @endforeach
            @endif
          @endif
        </td>
      ...
      <tfoot>
        <tr>
          <td colspan="3" class="text-center">
            ...
            @else
            <a href="{{ route('order.show', [$order->id]) }}" class="btn btn-primary">查看订单</a>
            @endif
          </td>
        </tr>
      </tfoot>
